...
let validate (cmd : Unwatch_bars_command.t) :
    (validated_unwatch_bars_command, validation_error) Rop.t =
  let open Rop in
  let+ instrument = parse_instrument cmd.symbol
  and+ timeframe = parse_timeframe cmd.timeframe in
  { instrument; timeframe }

let unwatch ~(broker : Broker.client) (v : validated_unwatch_bars_command) : unit =
  Broker.unsubscribe broker
    (Subscribe_bars { instrument = v.instrument; timeframe = v.timeframe })

let handle ~(broker : Broker.client) (cmd : Unwatch_bars_command.t) :
