module Order_leg_filled = Broker_integration_events.Order_leg_filled_integration_event

let handle
    ~(publish_order_leg_filled : Order_leg_filled.t -> unit)
    ~(origin_correlation_id : placement_id:int -> string option)
    (ev : Broker_domain.Remote_broker.Events.Order_leg_filled.t) : unit =
  ...
  | None ->
      Log.warn "[broker] fill for placement_id=%d has no Submit correlation_id — skipping"
        ev.placement_id
  | Some correlation_id ->
      publish_order_leg_filled (Order_leg_filled.of_domain ~correlation_id ev)
