...
let tcp_pair ~sw env =
  ...
  let listener =
    Eio.Net.listen ~reuse_addr:true ~backlog:1 ~sw net (`Tcp (loopback, 0))
  in
  let port =
...
let test_echo_roundtrip () =
  ...
  let uri = Uri.of_string (Printf.sprintf "ws://127.0.0.1:%d/echo" port) in
...
let tests = [ ("echo text roundtrip", `Quick, test_echo_roundtrip) ]
