...
let parse_instrument (vm : Instrument_view_model.t) : Core.Instrument.t option =
  let qualified =
    match vm.board with
    | Some b -> Printf.sprintf "%s@%s/%s" vm.ticker vm.venue b
    | None -> Printf.sprintf "%s@%s" vm.ticker vm.venue
  in
  try Some (Core.Instrument.of_qualified qualified) with Invalid_argument _ -> None

let parse_volume_bar ~ts (c : Candle_view_model.t) : Vb.t option =
  try
    let volume = Decimal.of_string c.volume in
    if Decimal.compare volume Decimal.zero < 0 then None else Some (Vb.make ~ts ~volume)
...
let parse_market_data_quote ~ts (c : Candle_view_model.t) : Mq.t option =
  try
    ...
    else Some (Mq.make ~ts ~bid:close ~ask:close ~realised_volatility:0.0)
  with Invalid_argument _ -> None
...
let handle
  ...
  | Some instrument -> (
      ...
      | None -> ()
      | Some bar -> deliver_volume_bar ~instrument ~timeframe:ev.timeframe ~bar);
