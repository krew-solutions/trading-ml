...
module Handler =
  Execution_management_external_integration_events.Bar_updated_integration_event_handler
...
let make_ev
    ...
    ?(volume = "1000")
    ?(timeframe = "1m")
    ?(ts = "2024-01-15T13:45:00Z")
    () : Ie.t =
  { instrument = sber_vm; timeframe; candle = { ts; open_; high; low; close; volume } }
...
let fans_out_volume_and_market_data () =
  ...
  Alcotest.(check string) "volume forwarded" "1000" (Decimal.to_string bar.volume);
...
let drops_when_close_is_zero () =
  ...
  Handler.handle
    ~deliver_volume_bar:(fun ~instrument:_ ~timeframe:_ ~bar:_ -> incr vol_calls)
...
let tests =
  [
    ...
    Alcotest.test_case "close ≤ 0 drops market_data only" `Quick drops_when_close_is_zero;
