...
let bar ~vol = Vb.make ~ts:0L ~volume:(Decimal.of_string vol)

let delivers_to_matching_subscriber () =
  let t = Feed.create () in
  let received = ref [] in
  let _ : Feed.subscription =
    Feed.subscribe t ~instrument:instrument_sber ~timeframe:"1m" ~on_bar:(fun b ->
        received := b :: !received)
  in
...
let does_not_deliver_to_other_instrument () =
  ...
  let _ : Feed.subscription =
    Feed.subscribe t ~instrument:instrument_sber ~timeframe:"1m" ~on_bar:(fun _ ->
        incr received)
  in
  Feed.deliver t ~instrument:instrument_gazp ~timeframe:"1m" ~bar:(bar ~vol:"100");
...
let does_not_deliver_to_other_timeframe () =
  ...
  let _ : Feed.subscription =
    Feed.subscribe t ~instrument:instrument_sber ~timeframe:"1m" ~on_bar:(fun _ ->
        incr received)
  in
  Feed.deliver t ~instrument:instrument_sber ~timeframe:"5m" ~bar:(bar ~vol:"100");
  Alcotest.(check int) "no delivery" 0 !received
...
let two_subscribers_same_key_both_fire () =
  ...
  let b = ref 0 in
  let _ : Feed.subscription =
    Feed.subscribe t ~instrument:instrument_sber ~timeframe:"1m" ~on_bar:(fun _ -> incr a)
  in
  let _ : Feed.subscription =
    Feed.subscribe t ~instrument:instrument_sber ~timeframe:"1m" ~on_bar:(fun _ -> incr b)
  in
  Feed.deliver t ~instrument:instrument_sber ~timeframe:"1m" ~bar:(bar ~vol:"100");
...
let unsubscribe_stops_deliveries () =
  ...
  Feed.deliver t ~instrument:instrument_sber ~timeframe:"1m" ~bar:(bar ~vol:"200");
