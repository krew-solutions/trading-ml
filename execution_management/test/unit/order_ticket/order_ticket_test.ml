...
let full_fill quantity_s =
  Placement.Values.Fill_record.make ~quantity:(qty quantity_s)
    ~price:(Decimal.of_string "250") ~fee:(Decimal.of_string "0.5") ~ts:1_700_000_000L

let count_kind events filter = List.length (List.filter filter events)
...
let test_open_immediate_emits_opened_plus_one_dispatched () =
  ...
  Alcotest.(check int)
    ...
    (count_kind events (function
      ...
      | _ -> false));
  Alcotest.(check bool)
    "lifecycle is Working" true
    (match Ot.lifecycle t with
    | Working _ -> true
    | _ -> false);
  Alcotest.(check int) "1 placement on ticket" 1 (List.length (Ot.placements t))
...
let test_immediate_full_fill_completes_ticket () =
  ...
  let pid =
    match
      List.find_map
        (function
          | Ot.Ev_placement_dispatched (e : Ot.Events.Placement_dispatched.t) ->
              Some e.placement_id
          | _ -> None)
        events
    with
  ...
  let t, events =
    Ot.on_placement_fill t ~placement_id:pid ~fill:(full_fill "100") ~now:1_700_000_002L
  in
  Alcotest.(check int)
    ...
    (count_kind events (function
      ...
      | _ -> false));
  Alcotest.(check bool)
    "lifecycle is Filled" true
    (match Ot.lifecycle t with
    | Filled -> true
    | _ -> false);
  Alcotest.(check string)
    "Σ filled = total" "100"
    ( Decimal.to_string (Values.Progress.remaining_quantity (Ot.progress t)) |> fun s ->
      if s = "0" then "100" else "" );
...
let test_partial_then_full_fill_completes_only_at_end () =
  let t, events =
    Ot.open_ticket ~ticket_id:ticket_id_42 ~reservation_id:reservation_id_42
      ~intent:(intent_buy_100 ()) ~directive:Values.Execution_directive.Immediate ~now:0L
  in
  let pid =
    match
      List.find_map
        (function
          | Ot.Ev_placement_dispatched e -> Some e.placement_id
          | _ -> None)
        events
    with
    | Some p -> p
  ...
  Alcotest.(check int)
    "no completed on partial" 0
    (count_kind evs1 (function
      | Ot.Ev_ticket_completed _ -> true
      | _ -> false));
  let t, evs2 = Ot.on_placement_fill t ~placement_id:pid ~fill:(full_fill "60") ~now:2L in
  Alcotest.(check int)
    "completed on second (final) fill" 1
    (count_kind evs2 (function
      | Ot.Ev_ticket_completed _ -> true
      | _ -> false));
  Alcotest.(check bool)
...
let test_immediate_rejection_terminates_failed () =
  ...
  in
  Alcotest.(check int)
    "Ticket_failed emitted" 1
    (count_kind evs (function
      | Ot.Ev_ticket_failed _ -> true
      | _ -> false));
  Alcotest.(check bool)
...
let test_twap_tick_emits_slice () =
  ...
  Alcotest.(check int)
    "1 placement dispatched on first tick" 1
    (count_kind events (function
      | Ot.Ev_placement_dispatched _ -> true
      | _ -> false))
...
let test_cancel_then_placement_cancelled_completes_to_cancelled () =
  ...
  Alcotest.(check int)
    "Ticket_cancelled emitted when last outstanding cancels" 1
    (count_kind evs (function
...
let tests =
  [
    ...
    Alcotest.test_case "TWAP tick emits scheduled slice" `Quick test_twap_tick_emits_slice;
    Alcotest.test_case "cancel emits Ticket_cancelling_started" `Quick
      test_cancel_emits_cancelling_started;
    Alcotest.test_case "cancel then placement_cancelled → Ticket_cancelled" `Quick
