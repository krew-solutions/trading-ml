...
let is_params
    ...
    ~risk_aversion
    ~temp_impact_eta =
  Values.Implementation_shortfall_params.make ~n_slices ~window_seconds ~start_at
    ~volatility ~risk_aversion ~temp_impact_eta
...
let drain_schedule ~intent ~params =
  ...
  for i = 1 to n do
    let due_at = Int64.add params.start_at (Int64.of_int (window * i / n)) in
...
let test_trajectory_sums_to_total () =
  ...
  Alcotest.(check string)
    "Σ slice_qty = total (residue construction)" "1000" (Decimal.to_string total);
  Alcotest.(check bool)
    "complete after all slices emitted" true (Is_strat.is_complete state)
...
let test_zero_volatility_degenerates_to_linear () =
  ...
  Alcotest.(check string) "σ = 0 → Σ still equals total" "100" (Decimal.to_string total)
...
let test_high_volatility_front_loads () =
  ...
  Alcotest.(check bool)
    "first slice ≥ second slice (front-loaded)" true
...
let test_price_quote_is_ignored_in_pr2 () =
  ...
  let quote =
    Values.Market_data_quote.make ~ts:1_010L ~bid:(Decimal.of_string "250")
      ~ask:(Decimal.of_string "251") ~realised_volatility:0.10
  in
  let _state', decision =
    Is_strat.on_event state (Input.Price_quote { quote }) ~now:1_010L
  in
  Alcotest.(check int) "Price_quote ignored in PR2" 0 (List.length decision.submit)

let tests =
  [
    Alcotest.test_case "trajectory sums to total" `Quick test_trajectory_sums_to_total;
