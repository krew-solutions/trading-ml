...
module Make (C : sig
  ...
  let init () = { prev_close = None; samples = 0; seed_sum = 0.0; value = None }
    ...
    | Some pc ->
        ...
        if st.samples < C.period then
          ...
          if samples = C.period then
            let v = seed_sum /. n in
            ({ prev_close = Some close; samples; seed_sum; value = Some v }, Some v)
          else ({ st with prev_close = Some close; samples; seed_sum }, None)
        else
          let v =
            match st.value with
            | Some v -> v
