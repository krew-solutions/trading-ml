  ...
  let update st candle =
    ...
    let st =
      if Ring.is_full st.ring then
        ...
        { ring = Ring.push st.ring p; sum = st.sum +. p; sum_sq = st.sum_sq +. (p *. p) }
    in
    let out =
      if Ring.is_full st.ring then
        ...
        let sd = sqrt var in
        Some { middle = mean; upper = mean +. (C.k *. sd); lower = mean -. (C.k *. sd) }
      else None
    in
    (st, out)
