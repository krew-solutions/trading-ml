...
module Make (C : sig
  ...
  let () =
    if C.fast <= 0 || C.slow <= 0 then invalid_arg "ChaikinOsc: periods must be > 0";
    if C.fast >= C.slow then invalid_arg "ChaikinOsc: fast must be < slow"

  type ema_st = { samples : int; seed_sum : float; value : float option }

  type state = { ad : float; fast_ema : ema_st; slow_ema : ema_st }
  type output = float
  ...
  let step_ema period a st x =
    if st.samples < period - 1 then
      { samples = st.samples + 1; seed_sum = st.seed_sum +. x; value = None }
    else if st.samples = period - 1 then
      let s = st.seed_sum +. x in
      { samples = st.samples + 1; seed_sum = s; value = Some (s /. float_of_int period) }
    else
      let v =
        match st.value with
        | Some v -> v
  ...
  let init () = { ad = 0.0; fast_ema = init_ema (); slow_ema = init_ema () }
