...
module Make (C : sig
  ...
  let name = Printf.sprintf "MACD(%d,%d,%d)" C.fast C.slow C.signal
  ...
  let step_ema period a samples seed ema x =
    if samples < period - 1 then (samples + 1, seed +. x, None)
  ...
  let update st candle =
    ...
    let gs, gseed, gema, macd =
      ...
      | Some f, Some s ->
          let macd = f -. s in
          let gs, gseed, gema =
            step_ema C.signal asig st.signal_samples st.signal_seed st.signal_ema macd
          in
          (gs, gseed, gema, Some macd)
      | _ -> (st.signal_samples, st.signal_seed, st.signal_ema, None)
    in
    let st' =
      {
        fast_samples = fs;
