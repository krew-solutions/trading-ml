...
let specs : spec list =
  [
    ...
    };
    {
      ...
      build =
        (fun p ->
          Macd.make ~fast:(get_int p "fast" 12) ~slow:(get_int p "slow" 26)
    ...
    };
    {
      ...
      build =
        (fun p ->
          Macd_weighted.make ~fast:(get_int p "fast" 12) ~slow:(get_int p "slow" 26)
            ~signal:(get_int p "signal" 9) ());
    };
    {
      name = "BollingerBands";
      ...
      build =
        (fun p ->
          Bollinger.make ~period:(get_int p "period" 20) ~k:(get_float p "k" 2.0) ());
    };
    {
      name = "ATR";
      params = [ ("period", Int 14) ];
    ...
    { name = "OBV"; params = []; build = (fun _ -> Obv.make ()) };
    { name = "A/D"; params = []; build = (fun _ -> Ad.make ()) };
    {
      name = "ChaikinOscillator";
      params = [ ("fast", Int 3); ("slow", Int 10) ];
      build =
        (fun p ->
          Chaikin_oscillator.make ~fast:(get_int p "fast" 3) ~slow:(get_int p "slow" 10)
            ());
    };
    {
      name = "Stochastic";
      ...
      build =
        (fun p ->
          Stochastic.make ~k_period:(get_int p "k_period" 14)
