...
type state = { params : params; rsi : Indicators.Indicator.t; position : position }
...
let on_candle st instrument (c : Candle.t) =
  ...
  | Some v ->
      ...
      let action, position, reason =
        ...
        | Flat when v < p.lower ->
            (Signal.Enter_long, Long, Printf.sprintf "RSI %.2f < %.2f" v p.lower)
        | Flat when p.allow_short && v > p.upper ->
            (Signal.Enter_short, Short, Printf.sprintf "RSI %.2f > %.2f" v p.upper)
      ...
      let strength =
        match action with
        | Signal.Enter_long -> Float.min 1.0 ((p.lower -. v) /. p.lower)
