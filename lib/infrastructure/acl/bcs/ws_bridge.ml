...
let make ~env ~sw ~cfg ~auth : bridge =
  let authenticator =
    ...
    | Ok a -> Some a
    | Error m -> Log.warn "[bcs ws] CA load failed: %s" m; None
  in
...
let subscribe_bars (t : bridge) ~instrument ~timeframe
  ...
  if already then ()
    ...
    let label = Printf.sprintf "bcs ws %s/%s"
      (Instrument.to_qualified instrument)
      (Timeframe.to_string timeframe) in
    ...
    let conn = Websocket.Resilient.create
      ~env:t.env ~sw:t.sw ~config in
