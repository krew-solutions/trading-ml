...
let send t msg =
  if not t.closed then
    try Client.send_text t.client msg
    with _ -> ()
...
and reconnect t =
  ...
  while not !connected && not t.closed do
    Log.warn "[%s] disconnected — reconnecting in %.0fs"
      t.config.label !backoff;
    ...
    if not t.closed then begin
      (try
         ...
         backoff := 1.0
       with e ->
         Log.warn "[%s] reconnect failed: %s (retry in %.0fs)"
           t.config.label (Printexc.to_string e) !backoff;
