...
let validate (cmd : Watch_footprints_command.t) :
    (validated_watch_footprints_command, validation_error) Rop.t =
  let open Rop in
...
let handle
    ~(watch : instrument:Instrument.t -> boundary:Bar_boundary.t -> unit)
    (cmd : Watch_footprints_command.t) : (unit, handle_error) Rop.t =
  match validate cmd with
  | Error errs -> Error (List.map (fun e -> Validation e) errs)
  | Ok { instrument; boundary } ->
      watch ~instrument ~boundary;
      Rop.succeed ()
