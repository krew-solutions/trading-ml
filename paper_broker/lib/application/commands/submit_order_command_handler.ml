...
let handle
    ...
    (Pending_order.t * Order.Events.Order_accepted.t, handle_error) Rop.t =
  ...
  | Ok v ->
      ...
      let order, event =
        Order.make ~id ~client_order_id:id ~instrument:v.instrument ~side:v.side
          ~quantity:v.quantity ~kind:v.kind ~tif:v.tif ~created_ts ~placed_after_ts
      in
      let pending =
        Pending_order.make ~order ~reservation_id:cmd.reservation_id
          ~correlation_id:cmd.correlation_id
      in
      (match S.save store_handle pending with
      | `Ok -> ()
      | `Already_exists ->
          invalid_arg
            (Printf.sprintf
               "Submit_order_command_handler: next_order_id %S collided in store" id));
      Rop.succeed (pending, event)
