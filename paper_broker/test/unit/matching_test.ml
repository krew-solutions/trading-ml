...
let check_some msg ~expected got =
  match got with
  | Some d ->
      Alcotest.(check string) msg expected (Decimal.to_string d)
...
let check_none msg got =
  ...
  | None -> ()
  | Some d ->
      Alcotest.fail (msg ^ Printf.sprintf ": expected None, got Some %s" (Decimal.to_string d))
...
let test_limit_buy_fills_at_open_when_gapped () =
  ...
  let r =
    Matching.price_if_filled
      ~kind:(Order_kind.limit (dec "100")) ~side:Core.Side.Buy ~candle
  in
...
let test_limit_sell_fills_at_open_when_gapped () =
  ...
  let r =
    Matching.price_if_filled
      ~kind:(Order_kind.limit (dec "100")) ~side:Core.Side.Sell ~candle
  in
...
let test_stop_buy_fills_at_open_when_gapped () =
  ...
  let r =
    Matching.price_if_filled
      ~kind:(Order_kind.stop (dec "100")) ~side:Core.Side.Buy ~candle
  in
...
let test_stop_sell_fills_at_open_when_gapped () =
  ...
  let r =
    Matching.price_if_filled
      ~kind:(Order_kind.stop (dec "100")) ~side:Core.Side.Sell ~candle
  in
...
let tests =
  [
    ...
    ("limit sell: gap past fills at open", `Quick, test_limit_sell_fills_at_open_when_gapped);
