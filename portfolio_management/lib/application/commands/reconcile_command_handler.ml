...
type validated_command = { book_id : Shared.Book_id.t; computed_at : int64 }
...
let handle
    ~(target_portfolio_for : Shared.Book_id.t -> Target_portfolio.t option)
    ~(actual_portfolio_for : Shared.Book_id.t -> Actual_portfolio.t option)
