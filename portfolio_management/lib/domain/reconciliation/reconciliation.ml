...
let intent_for ~book_id ~instrument ~target_qty ~actual_qty : Shared.Trade_intent.t option
    =
  ...
  if Decimal.is_zero delta then None
    ...
    Some ({ book_id; instrument; side; quantity } : Shared.Trade_intent.t)
...
let union_instruments target_positions actual_positions =
  let rec go acc t a =
    match (t, a) with
    | [], [] -> List.rev acc
    | (tp : Shared.Target_position.t) :: ts, [] -> go (tp.instrument :: acc) ts []
    | [], (ap : Actual_portfolio.Values.Actual_position.t) :: as_ ->
        go (ap.instrument :: acc) [] as_
    | ( (tp : Shared.Target_position.t) :: ts,
        (ap : Actual_portfolio.Values.Actual_position.t) :: as_ ) ->
