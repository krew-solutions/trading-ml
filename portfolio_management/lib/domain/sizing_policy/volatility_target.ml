...
let size_scalar
  ...
  let target_qty =
    qty_of ~target_annual_vol:cfg.target_annual_vol ~book_equity
      ~mark_price:(mark instrument) ~weight ~instrument_vol:(volatility instrument)
  in
...
let size
  ...
  | Common.Construction_intent.Scalar s ->
      size_scalar cfg ~book_equity ~mark ~volatility ~book_id:s.book_id
        ~instrument:s.instrument ~direction:s.direction ~strength:s.strength
        ~source:s.source ~observed_at:s.observed_at
  | Common.Construction_intent.Coupled c ->
      size_coupled cfg ~book_equity ~mark ~volatility ~book_id:c.book_id ~legs:c.legs
