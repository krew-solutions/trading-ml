...
let well_formed_cmd ?(alpha = "momentum-1") ?(inst = "SBER@MISX")
    ?(book = "book-α") () : SBA.t =
...
let test_happy_path_persists () =
  ...
  | Ok () ->
      ...
      Alcotest.(check string) "alpha source" "momentum-1"
        (Pm.Common.Alpha_source_id.to_string sub.alpha_source_id);
      Alcotest.(check string) "book" "book-α"
        (Pm.Common.Book_id.to_string sub.book_id)
...
let test_different_triplets_coexist () =
  let bag, persist = make_registry () in
  let _ =
    H.handle ~persist_subscription:persist
      (well_formed_cmd ~book:"book-α" ())
  in
  let _ =
    H.handle ~persist_subscription:persist (well_formed_cmd ~book:"book-β" ())
  in
  Alcotest.(check int) "both kept" 2 (List.length !bag)

let test_rejects_empty_alpha () =
  expect_validation_failure (well_formed_cmd ~alpha:"" ())

let test_rejects_malformed_instrument () =
  expect_validation_failure (well_formed_cmd ~inst:"bad" ())

let test_rejects_empty_book () =
  expect_validation_failure (well_formed_cmd ~book:"" ())

let tests =
  [
    Alcotest.test_case "happy path persists subscription" `Quick
      test_happy_path_persists;
    Alcotest.test_case "duplicate command is idempotent" `Quick
      test_duplicate_is_idempotent;
    Alcotest.test_case "different triplets coexist" `Quick
      test_different_triplets_coexist;
    Alcotest.test_case "rejects empty alpha_source_id" `Quick
      test_rejects_empty_alpha;
