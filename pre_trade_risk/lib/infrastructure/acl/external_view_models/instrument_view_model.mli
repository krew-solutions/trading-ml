(** Inbound DTO mirror of an instrument view model.

    Structural-only: identifies the four wire fields. No
    [of_domain] / [type domain] — this DTO is consumed (deserialized
    from an upstream BC's outbound JSON), not produced from a local
    domain value. Kept independent of
    {!Pre_trade_risk_view_models.Instrument_view_model} so the
    inbound and outbound sides of the wire can evolve their schemas
    independently.
