...
module Make (C : sig
  ...
  let step_ema st x =
    if st.ema_samples < C.period - 1 then
      { st with ema_samples = st.ema_samples + 1; ema_seed = st.ema_seed +. x }
    else if st.ema_samples = C.period - 1 then
      let s = st.ema_seed +. x in
      {
        st with
        ema_samples = st.ema_samples + 1;
        ema_seed = s;
  ...
  let update st c =
    let range = Decimal.to_float c.Candle.high -. Decimal.to_float c.low in
    ...
    let out =
      if Ring.size h = C.period + 1 then
        ...
        if prev = 0.0 then None else Some (100.0 *. (curr -. prev) /. prev)
      else None
    in
    (st'', out)
