...
let init p =
  if p.fast <= 0 || p.slow <= 0 then invalid_arg "Chaikin_Momentum: periods > 0";
  if p.fast >= p.slow then invalid_arg "Chaikin_Momentum: fast < slow";
  {
    params = p;
  ...
  | Some v ->
      let action, position, reason =
        ...
        | Some prev, Long when prev >= 0.0 && v < 0.0 ->
            if st.params.allow_short then
              (Signal.Enter_short, Short, "Chaikin osc crossed below zero (flip)")
