...
let run_live instrument candles =
  let paper =
    Paper.Paper_broker.make ~initial_cash:equity_init ~fee_rate
      ~source:(mk_stub_source ()) ()
  in
  let strat = Strategies.Strategy.default (module Strategies.Sma_crossover) in
...
let test_fills_match () =
  ...
  Alcotest.(check int)
    "same number of fills" (List.length r.fills) (List.length paper_fills);
  List.iter2
    (fun (bt : Engine.Backtest.fill) (p : Paper.Paper_broker.fill) ->
      Alcotest.(check string) "side" (Side.to_string bt.side) (Side.to_string p.side);
      Alcotest.(check (float 1e-6))
        "qty"
        (Decimal.to_float bt.quantity)
        (Decimal.to_float p.quantity);
      Alcotest.(check (float 1e-6))
        "price" (Decimal.to_float bt.price) (Decimal.to_float p.price);
      Alcotest.(check (float 1e-6))
        "fee" (Decimal.to_float bt.fee) (Decimal.to_float p.fee))
    r.fills paper_fills
...
let test_non_trivial_activity () =
  ...
  let r = run_backtest instrument candles in
  Alcotest.(check bool)
    "backtest produced at least one fill" true
    (List.length r.fills > 0)
