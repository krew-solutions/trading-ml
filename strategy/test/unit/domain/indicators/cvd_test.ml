...
let accumulates_on_close_at_high () =
  ...
  let ind =
    feed ind (List.init 5 (fun _ -> candle ~high:10.0 ~low:0.0 ~volume:100.0 10.0))
  in
...
let distributes_on_close_at_low () =
  let ind = Indicators.Cvd.make () in
  let ind =
    feed ind (List.init 3 (fun _ -> candle ~high:10.0 ~low:0.0 ~volume:100.0 0.0))
  in
...
let midpoint_is_zero_delta () =
  let ind = Indicators.Cvd.make () in
  let ind =
    feed ind (List.init 10 (fun _ -> candle ~high:10.0 ~low:0.0 ~volume:100.0 5.0))
  in
