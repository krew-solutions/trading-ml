...
let candle ?high ?low ?(volume = 1.0) close =
  ...
  let l =
    ...
    | None -> close
  in
  Candle.make ~ts:0L ~open_:(Decimal.of_float close) ~high:(Decimal.of_float h)
    ~low:(Decimal.of_float l) ~close:(Decimal.of_float close)
    ~volume:(Decimal.of_float volume)

let feed ind cs = List.fold_left (fun i c -> Indicators.Indicator.update i c) ind cs
