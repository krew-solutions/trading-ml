...
let up_down_mix () =
  ...
  let cs =
    [
      candle ~volume:10.0 100.0;
      (* seed, OBV = 0 *)
      candle ~volume:20.0 101.0;
      (* up → +20 *)
      candle ~volume:5.0 100.0;
      (* down → -5 *)
      candle ~volume:8.0 101.0;
      (* up → +8 *)
    ]
  in
...
let tests =
  [ ("up/down volume", `Quick, up_down_mix); ("flat closes → 0", `Quick, flat_closes) ]
