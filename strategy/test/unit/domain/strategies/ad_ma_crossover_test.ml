...
let build ?(period = 20) ?(allow_short = false) () =
  let p = Strategies.Ad_ma_crossover.{ period; allow_short } in
  Strategies.Strategy.make (module Strategies.Ad_ma_crossover) p
...
let test_uptrend_triggers_enter_long () =
  ...
  Alcotest.(check bool)
    "enter_long on A/D cross-up" true
...
let test_rejects_bad_period () =
  Alcotest.check_raises "period <= 0" (Invalid_argument "AD_MA_Crossover: period > 0")
