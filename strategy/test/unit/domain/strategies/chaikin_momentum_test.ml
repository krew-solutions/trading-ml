...
let test_regime_flip_triggers_enter_long () =
  ...
  Alcotest.(check bool)
    "enter_long on zero-cross up" true
...
let test_rejects_fast_ge_slow () =
  Alcotest.check_raises "fast >= slow" (Invalid_argument "Chaikin_Momentum: fast < slow")
    (fun () -> ignore (build ~fast:10 ~slow:3 ()))

let test_rejects_non_positive () =
  Alcotest.check_raises "fast <= 0" (Invalid_argument "Chaikin_Momentum: periods > 0")
