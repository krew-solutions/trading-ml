...
let test_oversold_enters_long () =
  ...
  Alcotest.(check bool) "enter_long on oversold" true (contains Signal.Enter_long acts)
...
let test_overbought_shorts_when_allowed () =
  ...
  let acts = actions_from_ohlc strat candles in
  Alcotest.(check bool)
    "enter_short on overbought" true
...
let test_rejects_bad_thresholds () =
  Alcotest.check_raises "lower >= upper" (Invalid_argument "MFI_MR: lower >= upper")
