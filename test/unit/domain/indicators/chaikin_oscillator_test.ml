...
let flat_ad_is_zero () =
  ...
  let ind =
    feed ind (List.init 30 (fun _ -> candle ~high:10.0 ~low:0.0 ~volume:100.0 5.0))
  in
  ...
  let ind =
    feed ind
      (List.init 40 (fun i ->
           ...
           candle ~high:10.0 ~low:0.0 ~volume:100.0 c))
  in
  Alcotest.(check bool) "defined" true (not (Float.is_nan (scalar ind)))
