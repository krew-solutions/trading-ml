...
let close_at_high () =
  ...
  let ind = feed ind (List.init 20 (fun _ -> candle ~high:23.0 ~low:0.0 23.0)) in
...
let close_at_low () =
  let ind = Indicators.Stochastic.make ~k_period:14 ~d_period:3 () in
  let ind = feed ind (List.init 20 (fun _ -> candle ~high:23.0 ~low:0.0 0.0)) in
  match values ind with
...
let flat_range_fallback () =
  let ind = Indicators.Stochastic.make ~k_period:14 ~d_period:3 () in
  let ind = feed ind (List.init 20 (fun _ -> candle ~high:5.0 ~low:5.0 5.0)) in
  match values ind with
