...
let constant_atr ~n ~value = Array.make n (Some value)
...
let test_tp_hit_first () =
  ...
  Alcotest.(check (option int))
    "TP hit at bar 2 → class 2" (Some 2)
    (Triple_barrier.label ~arr ~atr ~i:0 ~tp_mult:2.0 ~sl_mult:1.0 ~timeout:4)
...
let test_sl_hit_first () =
  ...
  let arr =
    [|
      bar ~ts:0 ~close:100.0 ~high:100.5 ~low:99.5;
      bar ~ts:1 ~close:99.5 ~high:100.0 ~low:98.5;
      (* SL=99 hit *)
  ...
  let atr = constant_atr ~n:5 ~value:1.0 in
  Alcotest.(check (option int))
    "SL hit at bar 1 → class 0" (Some 0)
    (Triple_barrier.label ~arr ~atr ~i:0 ~tp_mult:2.0 ~sl_mult:1.0 ~timeout:4)
...
let test_both_hit_same_bar_sl_wins () =
  ...
  let arr =
    [|
      bar ~ts:0 ~close:100.0 ~high:100.5 ~low:99.5;
      bar ~ts:1 ~close:100.0 ~high:103.0 ~low:98.0;
      (* spans TP=102 AND SL=99 *)
    |]
  in
  let atr = constant_atr ~n:2 ~value:1.0 in
  Alcotest.(check (option int))
    "both-hit → class 0 (SL-wins tie-break)" (Some 0)
    (Triple_barrier.label ~arr ~atr ~i:0 ~tp_mult:2.0 ~sl_mult:1.0 ~timeout:1)
...
let test_anchor_bar_itself_is_not_tested () =
  ...
  let arr =
    [|
      bar ~ts:0 ~close:100.0 ~high:103.0 ~low:99.0;
      (* already past TP at entry close *)
      bar ~ts:1 ~close:99.1 ~high:99.5 ~low:98.5;
      (* SL *)
    |]
  in
  let atr = constant_atr ~n:2 ~value:1.0 in
  Alcotest.(check (option int))
    "bar [i] itself ignored; walk starts at i+1 → SL" (Some 0)
    (Triple_barrier.label ~arr ~atr ~i:0 ~tp_mult:2.0 ~sl_mult:1.0 ~timeout:1)
...
let test_zero_atr_rejected () =
  ...
  Alcotest.(check (option int))
    "zero ATR → None (degenerate bands)" None
    (Triple_barrier.label ~arr ~atr ~i:0 ~tp_mult:2.0 ~sl_mult:1.0 ~timeout:1)
...
let test_asymmetric_barriers () =
  ...
  Alcotest.(check (option int))
    "asym: neither hit → class 1" (Some 1)
    (Triple_barrier.label ~arr ~atr ~i:0 ~tp_mult:3.0 ~sl_mult:0.5 ~timeout:3)
...
let test_timeout_clipped_to_array_end () =
  ...
  let atr = constant_atr ~n:4 ~value:1.0 in
  Alcotest.(check (option int))
    "partial window (short tail) → class 1" (Some 1)
    (Triple_barrier.label ~arr ~atr ~i:2 ~tp_mult:2.0 ~sl_mult:1.0 ~timeout:10)
