...
let candle ?(volume = 1.0) close =
  ...
  Candle.make ~ts:0L ~open_:p ~high:p ~low:p ~close:p ~volume:(Decimal.of_float volume)
...
let ts_candle ~ts close =
  let p = Decimal.of_float close in
  Candle.make ~ts:(Int64.of_int ts) ~open_:p ~high:p ~low:p ~close:p
...
let ohlc_candles_from_prices ?(volume = 1000.0) prices =
  List.mapi
    (fun i close ->
      let open_ = if i = 0 then close else List.nth prices (i - 1) in
      let h = Float.max open_ close +. 0.1 in
      let l = Float.min open_ close -. 0.1 in
      Candle.make ~ts:(Int64.of_int i) ~open_:(Decimal.of_float open_)
        ~high:(Decimal.of_float h) ~low:(Decimal.of_float l)
...
let contains (target : Signal.action) acts = List.exists (fun a -> a = target) acts
