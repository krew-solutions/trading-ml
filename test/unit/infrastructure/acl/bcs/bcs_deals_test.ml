...
let test_decode_single_deal () =
  let order_num, exec = Rest.bcs_execution_of_json
    (Yojson.Safe.from_string sample_deal) in
  Alcotest.(check string) "orderNum stringified"
    "1234567890" order_num;
  Alcotest.(check (float 1e-6)) "tradeQuantity" 10.0
    (Decimal.to_float exec.Order.quantity);
  Alcotest.(check (float 1e-6)) "price" 150.50
    (Decimal.to_float exec.price);
  Alcotest.(check (float 1e-6)) "fee always 0 (no wire field)" 0.0
    (Decimal.to_float exec.fee);
  Alcotest.(check bool) "ts parsed" true
    (Int64.compare exec.ts 0L > 0)
...
let test_decode_tolerant_numerics () =
  ...
  Alcotest.(check (float 1e-6)) "quantity from float" 7.0
    (Decimal.to_float exec.Order.quantity);
  Alcotest.(check (float 1e-6)) "price from int" 200.0
    (Decimal.to_float exec.price)
...
let test_orderNum_as_string () =
  ...
  Alcotest.(check string) "string orderNum preserved"
    "9999999999999" order_num

let test_decode_records_list () =
  let j = Yojson.Safe.from_string (Printf.sprintf
    {| { "records": [ %s ], "totalRecords": 1, "totalPages": 1 } |}
    sample_deal) in
  ...
  Alcotest.(check string) "first orderNum"
    "1234567890" order_num
...
let test_doc_example_shape () =
  ...
  Alcotest.(check int64) "ts drops sub-second"
    1722268288L exec.Order.ts
...
let test_filter_correlates_by_order_num () =
  ...
  let deals = [
    "1001", { Order.ts = 1L; quantity = Decimal.of_int 3;
              price = Decimal.of_float 100.0; fee = Decimal.zero };
    "1001", { Order.ts = 2L; quantity = Decimal.of_int 7;
              price = Decimal.of_float 101.0; fee = Decimal.zero };
    "1002", { Order.ts = 3L; quantity = Decimal.of_int 5;
              price = Decimal.of_float 200.0; fee = Decimal.zero };
  ] in
  let only_1001 = List.filter_map
    (fun (n, exec) -> if n = "1001" then Some exec else None) deals in
