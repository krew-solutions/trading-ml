...
let test_subscribe_envelope () =
  ...
  Alcotest.(check int) "subscribeType=0"
    0 (match member "subscribeType" j with `Int n -> n | _ -> -1);
  Alcotest.(check int) "dataType=1 (candles)"
    1 (match member "dataType" j with `Int n -> n | _ -> -1);
  ...
  | `List [ instr ] ->
    Alcotest.(check string) "classCode"
      "TQBR" (member "classCode" instr |> to_string);
    Alcotest.(check string) "ticker"
      "SBER" (member "ticker" instr |> to_string)
  | _ -> Alcotest.fail "expected single-element instruments array"

let test_unsubscribe_envelope () =
  let j = Bcs.Ws.unsubscribe_last_candle_message
    ~class_code:"TQBR" ~ticker:"SBER" ~timeframe:Timeframe.H1 in
  let open Yojson.Safe.Util in
  Alcotest.(check int) "subscribeType=1"
    1 (match member "subscribeType" j with `Int n -> n | _ -> -1)
...
let test_decode_candle () =
  ...
  | Candle_ev { instrument; timeframe; candle } ->
    ...
    Alcotest.(check (option string)) "board from classCode"
    ...
    Alcotest.(check (float 1e-6)) "open" 244.20
      (Decimal.to_float candle.Candle.open_);
    Alcotest.(check (float 1e-6)) "close" 244.50
      (Decimal.to_float candle.Candle.close);
    Alcotest.(check (float 1e-6)) "volume" 3200.0
