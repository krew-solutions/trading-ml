...
let token_body token =
  Yojson.Safe.to_string (`Assoc [ "token", `String token ])
...
let test_first_call_triggers_sessions_post () =
  ...
  in
  let auth = Auth.make ~secret:"SECRET" ~transport
               ~base:(Uri.of_string "https://example.test") in
  ...
  Alcotest.(check bool) "path ends in /v1/sessions" true
    (String.length path >= String.length suf &&
     String.sub path (String.length path - String.length suf)
       (String.length suf) = suf);
  let body = match req.body with Some b -> b | None -> "" in
  let j = Yojson.Safe.from_string body in
  Alcotest.(check string) "body carries secret"
    "SECRET"
...
let test_jwt_cached_until_expiry () =
  ...
  in
  let auth = Auth.make ~secret:"S" ~transport
               ~base:(Uri.of_string "https://x.test") in
  let _ = Auth.current auth in
  let _ = Auth.current auth in
  let _ = Auth.current auth in
  Alcotest.(check int) "subsequent calls do not re-POST"
...
let test_refresh_when_expired () =
  ...
  let _ = Auth.current auth in
  let _ = Auth.current auth in
  Alcotest.(check int) "expired token triggers re-auth"
