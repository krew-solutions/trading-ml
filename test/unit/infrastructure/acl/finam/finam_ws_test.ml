...
let test_decode_bars_data () =
  ...
  | Bars { instrument; timeframe; bars } ->
    ...
    Alcotest.(check bool) "timeframe from subscription_key"
      true (timeframe = Some Timeframe.M1);
...
let test_decode_bars_without_subscription_key () =
  ...
  | Bars { instrument; timeframe; bars = _ } ->
    Alcotest.(check string) "ticker recovered from payload"
      "GAZP" (Ticker.to_string (Instrument.ticker instrument));
    Alcotest.(check bool) "timeframe None without sub-key"
      true (timeframe = None)
  | _ -> Alcotest.fail "expected Bars event"
...
let test_decode_bars_spec_format () =
  ...
  | Bars { instrument; timeframe; bars } ->
    ...
    Alcotest.(check (float 1e-6)) "close" 301.0
      (Decimal.to_float c.Candle.close);
    Alcotest.(check (float 1e-6)) "volume" 12345.0
      (Decimal.to_float c.Candle.volume)
  | _ -> Alcotest.fail "expected Bars event"
