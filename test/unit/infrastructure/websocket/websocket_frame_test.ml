...
let test_roundtrip_text_small () =
  let f = { Websocket.Frame.fin = true; opcode = Text; payload = "hello" } in
...
let test_roundtrip_masked () =
  let f = { Websocket.Frame.fin = true; opcode = Text; payload = "masked!" } in
...
let test_mask_involution () =
  ...
  let once = Websocket.Frame.mask_payload ~key s in
  let twice = Websocket.Frame.mask_payload ~key once in
